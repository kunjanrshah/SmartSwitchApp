...
import com.espressif.esptouch.android.activities.HomeActivity;
...
public class ConnectionManager {
    ...
    public static void makeConnectionHolder(final Context givenCont) {
        if (!isMade) {
            context = givenCont;
            Intent homeIntent = new Intent(givenCont, HomeActivity.class);
            ...
            mqat.setCallback(new MqttCallback() {
                ...
                public void messageArrived(String s, MqttMessage mqttMessage) throws Exception {
                    ...
                    for (int i = 0; i < mqttMessage.toString().length(); i += chunkSize) {
                        str=mqttMessage.toString().substring(i, Math.min(mqttMessage.toString().length(), i + chunkSize));
                        Log.d("response", str);
                    }
                    Log.d("response kunjan", str);
                    if (s.contains("topic1")) {  //You can use equals if you like
                        //Do Some Stuff

                    }

                    if (s.contains("kunjan/send")) {
                        messageReceived.msgReceived(str);
    ...
    public static void setCallback(MessageReceived received){
